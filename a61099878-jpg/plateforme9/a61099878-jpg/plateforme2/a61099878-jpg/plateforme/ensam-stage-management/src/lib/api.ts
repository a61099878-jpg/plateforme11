...
class APIClient {
  ...
  async cancelConvention(conventionId: number, reason: string) {
    return this.request(`/student/convention/${conventionId}/cancel`, {
      method: 'POST',
      body: JSON.stringify({ reason }),
    });
  }

  async regenerateConvention() {
    const response = await this.request('/student/convention/regenerate', {
      method: 'POST',
    });
    return response.text();
  }
