...
export const conventions = sqliteTable('conventions', {
  ...
  status: text('status').default('en_attente'), // en_attente, envoye, valide, rejete, annule
  generatedAt: text('generated_at').default(sql`(datetime('now', 'localtime'))`),
  submittedAt: text('submitted_at'),
  validatedAt: text('validated_at'),
  rejectedAt: text('rejected_at'),
  cancelledAt: text('cancelled_at'), // nouvelle colonne pour la date d'annulation
  adminNotes: text('admin_notes'),
  cancellationReason: text('cancellation_reason'), // raison de l'annulation
});
...
// Types pour les statuts de convention
export type ConventionStatus = 'en_attente' | 'envoye' | 'valide' | 'rejete' | 'annule';
