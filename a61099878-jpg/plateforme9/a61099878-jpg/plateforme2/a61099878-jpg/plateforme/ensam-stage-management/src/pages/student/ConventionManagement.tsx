...
export function ConventionManagement() {
  ...
  const [cancellingConventionId, setCancellingConventionId] = useState<number | null>(null);
  ...
  const cancelConventionMutation = useMutation({
    mutationFn: ({ conventionId, reason }: { conventionId: number; reason: string }) => 
      api.cancelConvention(conventionId, reason),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['student', 'conventions'] });
      toast.success('Convention annulée avec succès');
    },
    onError: (error) => {
  ...
  const regenerateConventionMutation = useMutation({
    mutationFn: () => api.regenerateConvention(),
    onSuccess: (htmlContent) => {
      // Open HTML content in new tab for printing/PDF conversion
      const newWindow = window.open('', '_blank');
      if (newWindow) {
        ...
        queryClient.invalidateQueries({ queryKey: ['student', 'conventions'] });
        toast.success('Nouvelle convention générée ! Utilisez le bouton "Imprimer / Sauver en PDF" dans le nouvel onglet.');
      } else {
        toast.error('Veuillez autoriser les pop-ups pour générer votre convention.');
      }
    },
    onError: (error) => {
      toast.error(error.message || 'Erreur lors de la régénération de la convention');
    }
  });
    ...
    switch (status) {
      ...
      case 'annule':
        return {
          ...
          icon: XCircle,
          label: 'Annulée',
          description: 'Convention annulée par l\'étudiant',
          canUpload: false,
          progress: 0
        };
      default:
  ...
  const handleCancelConvention = (conventionId: number) => {
    if (!cancellationReason.trim()) {
      toast.error('Veuillez indiquer une raison pour l\'annulation');
      return;
    }
    
    cancelConventionMutation.mutate({ 
      conventionId, 
      reason: cancellationReason 
    });
    setCancellingConventionId(null);
    setCancellationReason('');
  };

  const handleRegenerateConvention = () => {
    regenerateConventionMutation.mutate();
  };

  // Permettre la génération si aucune convention ou si la dernière convention est annulée
  const lastConvention = conventions[0]; // Les conventions sont triées par date décroissante
  const canGenerateNewConvention = !lastConvention || lastConvention.status === 'annule';
  const hasActiveConvention = lastConvention && lastConvention.status !== 'annule';
            ...
            >
              {generateConventionMutation.isPending ? (
                ...
                <>
                  ...
                  {conventions.length > 0 ? 'Générer Nouvelle Convention' : 'Générer Convention'}
                </>
              )}
            </Button>
          ) : (
            <Button 
              onClick={handleRegenerateConvention}
              disabled={regenerateConventionMutation.isPending}
              size="lg"
              variant="outline"
              className="border-primary text-primary hover:bg-primary/10"
            >
              {regenerateConventionMutation.isPending ? (
                <>
          ...
          <Card key={convention.id} className={`border ${statusInfo.bgColor}`}>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                {convention.cancelledAt && (
                  <div className="flex items-center space-x-2 text-sm">
                    <div className="w-2 h-2 bg-gray-500 rounded-full"></div>
                    <span>Convention annulée</span>
                    <span className="text-gray-500">
                      {format(new Date(convention.cancelledAt), 'dd/MM/yyyy à HH:mm')}
                    </span>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-wrap gap-2 pt-2 border-t">
                ...
                {/* Bouton d'annulation pour les conventions non validées */}
                {(convention.status === 'en_attente' || convention.status === 'envoye') && (
                  <Dialog open={cancellingConventionId === convention.id} onOpenChange={(open) => {
                    if (!open) {
                      setCancellingConventionId(null);
                      setCancellationReason('');
                    }
                    ...
                    <DialogTrigger asChild>
                      <Button 
                        variant="destructive" 
                        size="sm"
                        onClick={() => setCancellingConventionId(convention.id)}
                      >
                        <XCircle className="h-4 w-4 mr-2" />
                        Annuler cette convention
                      </Button>
                    </DialogTrigger>
                    <DialogContent>
                      <DialogHeader>
                        <DialogTitle>Annuler la convention</DialogTitle>
                        <DialogDescription>
                          Êtes-vous sûr de vouloir annuler cette convention ? Cette action est irréversible.
                        </DialogDescription>
                      </DialogHeader>
                      <div className="space-y-4">
                        <div>
                          ...
                          <Textarea
                            ...
                            placeholder="Expliquez brièvement pourquoi vous annulez cette convention..."
                        ...
                        <div className="flex justify-end space-x-2">
                          ...
                          <Button 
                            variant="destructive" 
                            onClick={() => handleCancelConvention(convention.id)}
                            disabled={cancelConventionMutation.isPending}
                          >
                            {cancelConventionMutation.isPending ? (
                              <>
                                <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>
                                Annulation...
                              </>
