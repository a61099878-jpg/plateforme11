...
import { generateConventionPDF, generateConventionHTML, getStageType } from './services/pdf';
...
app.get('/api/student/convention/generate', async (c) => {
  try {
    ...
    return c.json({ error: 'Erreur lors de la génération de la convention' }, 500);
  }
});
...
app.post('/api/student/convention/:id/cancel', async (c) => {
  try {
    const user = await getCurrentUser(c);
    if (!user || user.role !== 'student') {
      return c.json({ error: 'Accès non autorisé' }, 403);
    ...
    // Check if convention can be cancelled (not already validated)
    if (convention.status === 'valide') {
      return c.json({ error: 'Impossible d\'annuler une convention déjà validée' }, 400);
    }
    
    // Update convention status to cancelled
    const updatedConvention = await db.update(schema.conventions)
      .set({
        status: 'annule',
        cancelledAt: new Date().toISOString(),
        cancellationReason: reason || 'Aucune raison fournie'
      })
      .where(eq(schema.conventions.id, conventionId))
      .returning();
    ...
    return c.json({ 
      ...
      convention: updatedConvention[0] 
    });
  } catch (error) {
...
app.post('/api/student/convention/regenerate', async (c) => {
  try {
    const user = await getCurrentUser(c);
    if (!user || user.role !== 'student') {
      return c.json({ error: 'Accès non autorisé' }, 403);
    ...
    // Check if student has any existing conventions that are not cancelled
    const existingConvention = await db.query.conventions.findFirst({
      where: eq(schema.conventions.studentId, user.id),
      orderBy: [desc(schema.conventions.generatedAt)]
    });
    
    if (existingConvention && existingConvention.status !== 'annule') {
      return c.json({ error: 'Vous avez déjà une convention active. Annulez-la d\'abord pour en générer une nouvelle.' }, 400);
    }
    
    const htmlContent = await generateConventionHTML(student);
    const typeStage = getStageType(student.annee);
    
    // Save new convention record
    await db.insert(schema.conventions).values({
      studentId: student.id,
      typeStage,
      status: 'en_attente'
    ...
    return c.json({ error: 'Erreur lors de la régénération de la convention' }, 500);
  }
});
