...
export function ConventionsManagement() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'annule':
        return <Badge variant="outline" className="text-gray-600 border-gray-300"><XCircle className="h-3 w-3 mr-1" />Annulée</Badge>;
      default:
  ...
  const stats = {
    ...
    cancelled: conventions.filter((c: Convention) => c.status === 'annule').length,
  };
  ...
  const conventionsByStatus = {
    ...
    annule: conventions.filter((c: Convention) => c.status === 'annule'),
  };
    ...
    <div className="space-y-6">
      ...
      <Tabs value={selectedStatus} onValueChange={setSelectedStatus}>
        <TabsList>
          ...
          <TabsTrigger value="annule">Annulées ({stats.cancelled})</TabsTrigger>
        </TabsList>
